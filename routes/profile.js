...
router.patch("/update-location", async (req, res) => {
  //Axios get request to get location
  ...
  try {
    ...
    // The response data will contain the location information in JSON format
    ...
    const user = await UsersSchema.findById(check.data._id);
    if (user) {
      user.location = locationData.results[0].formatted_address;
      user.save();
    }
